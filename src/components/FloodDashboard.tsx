...
import { ISIOLO_CENTER, HISTORICAL_FLOOD_ZONES, SAFE_HOUSES } from '../config/telemetryConfig';
...
const HeatmapLayer: React.FC<{ points: HeatmapPoint[]; radius: number; opacity: number }> = ({ points, radius, opacity }) => {
  ...
  useEffect(() => {
    ...
    // Debounce the map events to avoid heavy execution under rapid interactions
    let timeoutId: any = null;
    const debouncedUpdateHeatmap = () => {
    ...
    map.on('moveend zoomend resize', debouncedUpdateHeatmap);
    return () => {
      if (timeoutId) clearTimeout(timeoutId);
      map.off('moveend zoomend resize', debouncedUpdateHeatmap);
    };
